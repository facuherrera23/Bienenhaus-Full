...
export const OwnerFormSchema = z
    .object({
        ...
        dni_cuit: z
            .string()
            .regex(/^[\d\-\.]{8,13}$/, 'DNI/CUIT inválido')
            .optional()
            .nullable(),
...
export async function validateOwnershipSum(
    ...
    if (sum < 100 && links.length > 0)
        errors.push(`Suma ownership ${sum}% < 100% (falta distribuir)`);
...
async function processDocument(doc: OwnerDocument) {
    ...
    const {
        data: { text },
    } = await worker.recognize(doc.file_url);
    ...
    await supabase
        ...
        .update({
            ...
            expiry_date: extracted.expiry_date
                ? new Date(extracted.expiry_date).toISOString().split('T')[0]
                : null,
        })
        .eq('id', doc.id);
}
