...
export async function upsertSetting(
    ...
    if (existing) {
        await supabase
            .from('site_settings')
            .update({ value: value as any, value_type: valueType, is_public: isPublic })
            .eq('id', existing.id);
    } else {
        await supabase
            .from('site_settings')
...
export function getNextWhatsAppUrl(
    settings: SiteSettings,
    context?: { visitorCountry?: string; visitorCity?: string },
): string {
    ...
    if (routing.type === 'schedule' && routing.config.schedule) {
        ...
        return isPrimaryTime
            ? toWhatsAppUrl(routing.primary_phone)
            : toWhatsAppUrl(routing.alt_phone ?? routing.primary_phone);
    }
