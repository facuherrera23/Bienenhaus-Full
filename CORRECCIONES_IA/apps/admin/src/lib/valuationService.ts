...
export async function update(
    ...
    if (data.comparables !== undefined || data.f_fotoFachada !== undefined) {
        const saved = data.comparables !== undefined
            ? await replaceComparables(id, data.comparables)
            : await fetchComparables(id);
        const fachadaUrl = data.f_fotoFachada !== undefined
            ? data.f_fotoFachada
            : current.f_fotoFachada;
        await syncImages(
            id,
            fachadaUrl,
            saved.map((c) => ({ id: c.id, orden: c.orden, fotoUrl: c.fotoUrl })),
        );
    }
