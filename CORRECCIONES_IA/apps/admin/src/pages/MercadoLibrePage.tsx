...
export function MercadoLibrePage() {
    ...
    const connect = async () => {
        ...
        try {
            const authorizeUrl = await buildAuthorizeUrl(appIdDraft.trim());
            if (popup) popup.location.href = authorizeUrl;
            else window.location.assign(authorizeUrl);
        } catch (err) {
            popup?.close();
            pushToast({ type: 'error', title: 'No se pudo iniciar Mercado Libre', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
        }
    };
