...
describe('Select', () => {
  ...
  it('fires onChange with the option value when an option is clicked', () => {
    ...
    render(<Select options={FRUITS} placeholder="Pick" aria-label="Fruit" onChange={onChange} />);
  ...
  it('renders the error message with role=alert and aria-invalid when error is a string', () => {
    render(<Select options={FRUITS} error="Campo obligatorio" placeholder="Pick" aria-label="Fruit" />);
    const trigger = screen.getByRole('combobox', { name: 'Fruit' });
  ...
  it('adds the open modifier class to the root when the panel is open', () => {
    const { container } = render(<Select options={FRUITS} placeholder="Pick" aria-label="Fruit" />);
    const trigger = screen.getByRole('combobox', { name: 'Fruit' });
    fireEvent.click(trigger);
  ...
  it('smoke renders all sizes without throwing', () => {
    expect(() => render(<Select options={FRUITS} size="sm" placeholder="s" aria-label="s" />)).not.toThrow();
    expect(() => render(<Select options={FRUITS} size="md" placeholder="m" aria-label="m" />)).not.toThrow();
    expect(() => render(<Select options={FRUITS} size="lg" placeholder="l" aria-label="l" />)).not.toThrow();
  });
  ...
  it('controlled mode reflects the external value and does not mutate internally', () => {
    ...
    const { rerender } = render(
      <Select options={FRUITS} value="cherry" placeholder="Pick" aria-label="Fruit" onChange={onChange} />,
    );
    expect(screen.getByText('Cherry')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('combobox', { name: 'Fruit' }));
    fireEvent.click(screen.getByRole('option', { name: 'Banana' }));
    ...
    rerender(<Select options={FRUITS} value="banana" placeholder="Pick" aria-label="Fruit" onChange={onChange} />);
  ...
  it('renders a hidden input with the name and selected value', () => {
    const { container } = render(
      <Select options={FRUITS} name="fruit" defaultValue="banana" placeholder="Pick" aria-label="Fruit" />,
    );
  ...
  it('keeps the panel open after selecting in multi mode', () => {
    render(
      <Select options={FRUITS} multiple placeholder="Pick" aria-label="Fruit" />,
    );
    fireEvent.click(screen.getByRole('combobox', { name: 'Fruit' }));
    fireEvent.click(screen.getByRole('option', { name: 'Apple' }));
    expect(screen.getByRole('listbox')).toBeInTheDocument();
