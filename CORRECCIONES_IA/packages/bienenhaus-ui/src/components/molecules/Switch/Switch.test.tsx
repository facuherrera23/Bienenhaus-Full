...
describe('Switch', () => {
  ...
  it('controlled mode respects external checked prop', () => {
    const onChange = vi.fn();
    const { rerender } = render(<Switch aria-label="Toggle" checked={false} onChange={onChange} />);
    const input = screen.getByRole('switch');
    expect(input).toHaveAttribute('aria-checked', 'false');
  ...
  it('applies size classes (sm and md)', () => {
    const { container: smContainer, unmount: smUnmount } = render(<Switch aria-label="sm" size="sm" />);
