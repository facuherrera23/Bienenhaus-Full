...
async function optimize() {
    ...
    console.log(
        `Original: ${metadata.width}x${metadata.height}, ${(metadata.size / 1024 / 1024).toFixed(2)} MB, formato: ${metadata.format}\n`,
    );
    ...
    for (const size of sizes) {
        ...
        console.log(
            `  ✅ hero-baner-${size.suffix}.webp (${size.width}px) - ${(stats.size / 1024).toFixed(1)} KB`,
        );
    }
    ...
    for (const size of sizes) {
        ...
        const stats = fs.statSync(outputPath);
        console.log(
            `  ✅ hero-baner-${size.suffix}.avif (${size.width}px) - ${(stats.size / 1024).toFixed(1)} KB`,
        );
    }
