...
Deno.serve(async (req) => {
    ...
    if (req.method === 'POST') {
        ...
        if (error) {
            console.error('[audit-log] Error insertando entrada de auditoría:', error);
    ...
    if (url.searchParams.get('actor_id'))
        filters.actor_id = `eq.${url.searchParams.get('actor_id')}`;
    if (url.searchParams.get('entity_type'))
        filters.entity_type = `eq.${url.searchParams.get('entity_type')}`;
    if (url.searchParams.get('entity_id'))
        filters.entity_id = `eq.${url.searchParams.get('entity_id')}`;
    ...
    if (url.searchParams.get('from_date'))
        filters.created_at = `gte.${url.searchParams.get('from_date')}`;
    if (url.searchParams.get('to_date'))
        filters.created_at = `lte.${url.searchParams.get('to_date')}`;
    ...
    if (error) {
        console.error('[audit-log] Error listando entradas de auditoría:', error);
        return respond(500, { error: error.message });
    }
