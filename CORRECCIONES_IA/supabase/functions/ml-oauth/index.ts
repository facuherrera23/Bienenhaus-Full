...
async function hmac(value: string): Promise<string> {
    ...
    if (!secret) throw new Error('ML_CLIENT_SECRET no configurado');
...
Deno.serve(async (req) => {
    ...
    if (req.method === 'POST') {
        ...
        const signature = await hmac(`${nonce}|${admin.id}|${exp}`);
    ...
    try {
        ...
        const expected = await hmac(`${state.nonce}|${(await supabase.from('ml_oauth_states').select('admin_user_id').eq('nonce', state.nonce).maybeSingle()).data?.admin_user_id ?? ''}|${state.exp}`);
    ...
    try {
        const tokenResult = await runMlApiCallWithRetry(
            '',
            () => exchangeCode(code, redirectUri, oauthState.code_verifier),
            'exchangeCode',
        );
        if (!tokenResult.ok) {
