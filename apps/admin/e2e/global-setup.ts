...
async function seedFixtures(client: SupabaseClient): Promise<string[]> {
    ...
    const { data: agents, error: agentsError } = await client
        ...
        .select('id, email');
    if (agentsError)
        throw new Error(`global-setup: falló insert de agents: ${agentsError.message}`);
    ...
    const { data: props, error: propsError } = await client
        ...
        .select('id, slug');
    if (propsError)
        throw new Error(`global-setup: falló insert de properties: ${propsError.message}`);
    ...
    const imageRows = PROPERTY_FIXTURES.flatMap((p) => {
        ...
        return [
            { property_id: propertyId, url: img.url, alt: img.alt, position: 0, is_cover: true },
        ];
    });
    const { error: imagesError } = await client.from('property_images').insert(imageRows);
    if (imagesError)
        throw new Error(`global-setup: falló insert de property_images: ${imagesError.message}`);
