...
test.describe('Performance Audit', () => {
  for (const pg of pages) {
    test(`${pg.name} - Performance Metrics`, async ({ page: pwPage }) => {
      ...
      const totalJS = resources.filter(r => r.type === 'script').reduce((sum, r) => sum + r.size, 0);
      const totalCSS = resources.filter(r => r.type === 'style' || r.name.endsWith('.css')).reduce((sum, r) => sum + r.size, 0);
      const totalSize = resources.reduce((sum, r) => sum + r.size, 0);

      console.log(`Bundle - JS: ${(totalJS/1024).toFixed(1)}KB, CSS: ${(totalCSS/1024).toFixed(1)}KB, Total: ${(totalSize/1024).toFixed(1)}KB`);
