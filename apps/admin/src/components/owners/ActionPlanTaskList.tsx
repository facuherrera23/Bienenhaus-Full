import { Calendar, Clock, User, MoreVertical, Check } from 'lucide-preact';
import type { ActionPlanTaskRow, ActionPlanStatus } from '../../types/owners';
...
export function ActionPlanTaskList({ tasks, onToggle, onEdit, onDelete, canEdit = true }: ActionPlanTaskListProps) {
  ...
  return (
    <div className="action-plan-task-list">
      ...
      <ul className="task-list" role="list">
        {tasks.map((task) => (
          <li key={task.id} className={`task-item${task.status === 'completed' ? ' completed' : ''}${isOverdue(task) ? ' overdue' : ''}`}>
            <div className="task-main">
              <label className="task-checkbox">
<select
                   value={task.status}
                   onChange={(e) => onToggle(task.id, (e.target as HTMLSelectElement).value as ActionPlanStatus)}
            ...
            <div className="task-meta">
              {task.due_date && (
                <span className={`task-due-date${isOverdue(task) ? ' overdue' : ''}`}>
                  ...
                  {isOverdue(task) && <Clock size={12} className="overdue-icon" />}
                </span>
              )}
