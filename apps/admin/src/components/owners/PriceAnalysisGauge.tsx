...
import { PRICE_STATUS_GAUGE_COLOR, formatPriceStatus } from '../../types/owners';
...
export function PriceAnalysisGauge({ analysis, size = 200, showLabels = true }: PriceAnalysisGaugeProps) {
  if (!analysis) {
    return (
  ...
  const { price_difference_pct, price_status, estimated_market_price, our_listing_price } = analysis;
      ...
      {showLabels && (
        <div className="gauge-prices">
          <div className="price-item">
            ...
            <span className="price-value">${our_listing_price.toLocaleString('es-AR')}</span>
          </div>
          <div className="price-item">
            <span className="price-label">Precio mercado</span>
            <span className="price-value">${estimated_market_price.toLocaleString('es-AR')}</span>
          </div>
        </div>
      )}
    </div>
