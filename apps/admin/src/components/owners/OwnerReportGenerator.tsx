...
import { FileText, Calendar, TrendingUp, X, Plus, Check } from 'lucide-preact';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { reportSchema, type ReportFormValues, type ReportType } from '../../lib/owners/schemas';
import type { PriceAnalysisRow, ActionPlanRow, CommunicationRow } from '../../types/owners';
...
const REPORT_TEMPLATES: Record<ReportType, { icon: any; description: string; sections: string[] }> = {
  ...
  market_update: {
    ...
    sections: ['Tendencias de precio', 'Nuevas publicaciones', 'Ventas recientes', 'Análisis de demanda'],
  },
  weekly: {
    icon: Calendar,
...
export function OwnerReportGenerator({
  ...
  const handlePreview = () => {
    ...
    if (!methods.getValues('title')) {
      methods.setValue('title', `${REPORT_TYPE_LABEL[selectedType]} - ${new Date().toLocaleDateString('es-AR')}`);
    }
    setStep('preview');
  };
          ...
          return (
            ...
            >
              ...
              <ul className="sections-preview">
                {template.sections.map((s) => <li key={s}>{s}</li>)}
              </ul>
            </button>
          );
        })}
      ...
      <div className="report-step">
        ...
        {selectedType === 'custom' && (
          <div className="custom-sections">
            ...
            <div className="section-inputs">
                ...
                <div key={i} className="section-input-row">
                  <input
                    ...
                    onInput={(e) => {
                      ...
                      updated[i] = (e.currentTarget as HTMLInputElement).value;
                      setCustomSections(updated);
                    }}
                    placeholder={`Sección ${i + 1}`}
                  />
                  <button type="button" className="icon-btn icon-btn--danger" onClick={() => {
              ...
              <button type="button" className="btn btn--ghost btn--sm" onClick={() => setCustomSections([...customSections, ''])}>
            ...
            <div className="form-field">
              ...
              <textarea
                ...
                rows={6}
                value={customContent}
                onInput={(e) => setCustomContent((e.currentTarget as HTMLTextAreaElement).value)}
        ...
        <div className="included-data">
          ...
          <ul>
            ...
            {communications.length > 0 && <li>{communications.length} comunicación(es) reciente(s)</li>}
          </ul>
        </div>

        <div className="step-actions">
          <button type="button" className="btn btn--ghost" onClick={handleBack}>Volver</button>
  ...
  const renderStepPreview = () => (
    <div className="report-step report-preview">
      ...
      <div className="preview-content">
        <div className="preview-header">
          ...
          <div className="preview-meta">
            <span>Tipo: {REPORT_TYPE_LABEL[methods.getValues('report_type') as ReportType]}</span>
        ...
        <div className="preview-body">
            ...
            <div className="preview-section">
              ...
              <div className="price-summary">
                <div className="price-row">
                  ...
                  <strong>${priceAnalysis.our_listing_price.toLocaleString('es-AR')}</strong>
                </div>
                <div className="price-row">
                  <span>Precio mercado estimado:</span>
                  <strong>${priceAnalysis.estimated_market_price.toLocaleString('es-AR')}</strong>
                </div>
                <div className="price-row highlight">
                  <span>Diferencia:</span>
                  <strong className={`status-${priceAnalysis.price_status}`}>
                    {priceAnalysis.price_difference_pct >= 0 ? '+' : ''}{priceAnalysis.price_difference_pct.toFixed(2)}%
                  </strong>
                </div>
                <div className="price-row">
                  <span>Estado:</span>
                  <span className={`badge badge--${priceAnalysis.price_status}`}>{priceAnalysis.price_status}</span>
                </div>
                {priceAnalysis.recommendation && (
                  <div className="recommendation">
                    <strong>Recomendación:</strong> {priceAnalysis.recommendation}
                  </div>
                )}
              </div>
            </div>
          ...
          {(selectedType === 'weekly' || selectedType === 'monthly') && actionPlans.length > 0 && (
            <div className="preview-section">
            ...
            <div className="preview-section">
              ...
              <ul className="communications-preview">
                {communications.slice(0, 5).map((c) => (
                  <li key={c.id}>
                    <span>{c.type}: {c.subject || 'Sin asunto'}</span>
          ...
          {selectedType === 'custom' && customContent && (
            <div className="preview-section">
              ...
              <div className="custom-content-preview" dangerouslySetInnerHTML={{ __html: customContent }} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="step-actions">
        <button type="button" className="btn btn--ghost" onClick={handleBack}>Editar</button>
        <button
          type="button"
  ...
  return (
    <div className="owner-report-generator">
      ...
      <form onSubmit={handleSubmit} className="hidden-form">
        {methods.formState.errors.title && <span className="error">{methods.formState.errors.title.message}</span>}
      </form>
    </div>
  );
}