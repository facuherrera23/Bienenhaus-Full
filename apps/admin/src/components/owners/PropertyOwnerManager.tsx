...
import { Plus, Search, Trash2, Check, X } from 'lucide-preact';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { propertyOwnerLinkSchema, type PropertyOwnerLinkFormValues } from '../../lib/owners/schemas';
import {
  fetchPropertyOwners,
  linkOwnerToProperty,
...
export function PropertyOwnerManager({ propertyId, onOwnersChange }: PropertyOwnerManagerProps) {
  ...
  const unlinkMutation = useMutation({
    mutationFn: ({ ownerId }: { ownerId: string }) => unlinkOwnerFromProperty(propertyId, ownerId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-owners', propertyId] });
      queryClient.invalidateQueries({ queryKey: ['owners'] });
      onOwnersChange?.();
  ...
  const updateLinkMutation = useMutation({
    mutationFn: ({ ownerId, updates }: { ownerId: string; updates: Partial<PropertyOwnerLinkFormValues> }) =>
  ...
  const handleSubmit = async (data: PropertyOwnerLinkFormValues) => {
    ...
    methods.reset({ property_id: propertyId, owner_id: '', ownership_percentage: 100, is_primary_contact: false, role: 'propietario' });
  };

  const filteredOwners = allOwners?.filter(
    (o) => !linkedOwners?.some((lo) => lo.owner_id === o.id)
  ) ?? [];
                ...
                <tr key={owner.id}>
                  <td>
                    <div className="owner-cell">
                      <span className="owner-avatar-sm">{owner.owner_name?.charAt(0).toUpperCase()}</span>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => updateLinkMutation.mutate({ ownerId: owner.owner_id, updates: { ownership_percentage: parseFloat((e.currentTarget as HTMLInputElement).value) } })}
                  ...
                  <td>
                    <input
                      type="text"
                      value={owner.role}
                      onChange={(e) => updateLinkMutation.mutate({ ownerId: owner.owner_id, updates: { role: (e.currentTarget as HTMLInputElement).value } })}
                  ...
                  <td className="text-center">
                    <label className="checkbox-wrapper">
                      <input
                        ...
                        onChange={() => setPrimaryMutation.mutate(owner.owner_id)}
                        disabled={owner.is_primary_contact || setPrimaryMutation.isPending}
                      />
                      <span className="checkmark"></span>
                    </label>
                    {owner.is_primary_contact && <span className="primary-badge">Principal</span>}
                  </td>
                  <td>
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        if (window.confirm(`¿Desvincular a ${owner.owner_name}?`)) {
                          unlinkMutation.mutate({ ownerId: owner.owner_id });
                        }
                      }}
        ...
        <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={methods.handleSubmit(handleSubmit)} className="modal-body">
              ...
              {search.length >= 2 && filteredOwners.length === 0 && !loadingOwners && (
                <p className="hint">No se encontraron propietarios. Puedes crear uno nuevo desde <a href="/propietarios/nuevo">Propietarios → Nuevo</a>.</p>
              )}
              ...
              {search.length >= 2 && filteredOwners.length > 0 && (
                <div className="owner-search-results">
                  {filteredOwners.map((owner) => (
                    ...
                    >
                      <span className="owner-result-avatar">{owner.full_name.charAt(0).toUpperCase()}</span>
                      <div>
                        <strong>{owner.full_name}</strong>
                        <span className="muted">{owner.email || owner.phone || 'Sin contacto'}</span>
                      </div>
              ...
              {selectedOwnerId && (
                <div className="selected-owner-config">
                  ...
                  <div className="form-grid">
                    <div className="field">
                      <label htmlFor="ownership_percentage">% Propiedad</label>
                      <input
              ...
              <div className="modal-actions">
                ...
                >
                  {linkMutation.isPending ? 'Vinculando...' : 'Vincular propietario'}
                </button>
              </div>
            </form>
          </div>
