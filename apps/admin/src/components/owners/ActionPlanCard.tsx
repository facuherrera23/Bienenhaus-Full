import { Calendar, AlertCircle, User } from 'lucide-preact';
...
export function ActionPlanCard({ plan, tasks, onClick, onToggleTask }: ActionPlanCardProps) {
  ...
  return (
    <article className={`action-plan-card${isOverdue ? ' overdue' : ''}`} onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }}>
      <div className="card-header">
        ...
        <div className="plan-meta">
          ...
          <span className="category-badge">{ACTION_PLAN_CATEGORY_LABEL[plan.category]}</span>
        </div>
      </div>
      ...
      <div className="plan-progress">
        ...
        <span className="progress-text">{completedTasks} / {totalTasks} tareas</span>
      </div>

      {tasks.length > 0 && (
        <ul className="plan-tasks-preview">
          {tasks.slice(0, 3).map((task) => (
            <li key={task.id} className={`task-preview${task.status === 'completed' ? ' completed' : ''}`}>
              <input
                type="checkbox"
