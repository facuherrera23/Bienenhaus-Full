...
import { Plus, Trash2, GripVertical } from 'lucide-preact';
...
export function ComparablePropertyInput({ value, onChange, disabled }: ComparablePropertyInputProps) {
  ...
  const updateComparable = (index: number, field: keyof ComparableProperty, newValue: string | number | null) => {
    const updated = [...comparables];
    if (field === 'price' || field === 'sqm') {
      updated[index] = { ...updated[index], [field]: newValue === '' ? null : Number(newValue) };
    } else {
            ...
            <div className="comparable-fields">
              <div className="field">
                ...
                <input
                  ...
                  value={comp.address}
                  onInput={(e) => updateComparable(index, 'address', (e.currentTarget as HTMLInputElement).value)}
                  disabled={disabled}
                  required
                />
              </div>
              ...
              <div className="field">
                ...
                <input
                  ...
                  placeholder="150000"
                  value={comp.price ?? ''}
                  onInput={(e) => updateComparable(index, 'price', (e.currentTarget as HTMLInputElement).value)}
                  disabled={disabled}
                  required
                />
              </div>
              ...
              <div className="field">
                ...
                <input
                  ...
                  placeholder="80"
                  value={comp.sqm ?? ''}
                  onInput={(e) => updateComparable(index, 'sqm', (e.currentTarget as HTMLInputElement).value)}
                  disabled={disabled}
                />
              </div>
              ...
              <div className="field">
                ...
                <input
                  type="date"
                  value={comp.date}
                  onInput={(e) => updateComparable(index, 'date', (e.currentTarget as HTMLInputElement).value)}
                  disabled={disabled}
                  required
                />
              </div>
              ...
              <div className="field">
                ...
                <input
                  ...
                  value={comp.source ?? ''}
                  onInput={(e) => updateComparable(index, 'source', (e.currentTarget as HTMLInputElement).value)}
                  disabled={disabled}
                />
              </div>
            </div>
      ...
      {comparables.length > 0 && (
        <div className="comparable-summary">
          <p>
            ...
            Precio promedio: ${(comparables.reduce((sum, c) => sum + (c.price ?? 0), 0) / comparables.length).toLocaleString('es-AR')}
            {comparables.some(c => c.sqm) && (
              <>
                ...
                Precio promedio m²: ${(
                  comparables
                    .filter(c => c.sqm)
                    .reduce((sum, c) => sum + (c.price ?? 0) / (c.sqm ?? 1), 0) /
