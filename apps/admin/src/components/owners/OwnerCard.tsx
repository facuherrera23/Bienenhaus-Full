import { Building2, Mail, Phone, MapPin, UserCheck } from 'lucide-preact';
...
export function OwnerCard({ owner, onClick }: OwnerCardProps) {
  ...
  return (
    <article className="owner-card" onClick={handleClick} style={{ cursor: onClick ? 'pointer' : 'default' }}>
      ...
      <div className="owner-footer">
        <Link href={`/propietarios/${owner.id}`} className="owner-link" onClick={(e) => e.stopPropagation()}>
          Ver detalle
        </Link>
        <div className="owner-property-count">
          <Building2 size={14} />
          <span>{owner.property_count} propiedad{owner.property_count !== 1 ? 'es' : ''}</span>
        </div>
      </div>
    </article>
  );
