...
export const TipoInmuebleEnum = z.enum([
  'CASA', 'DEPTO', 'LOTE', 'GALPON', 'OFICINA', 'LOCAL', 'OTRO'
]);

export const DestinoEnum = z.enum(['Venta', 'Alquiler']);

export const NivelCalidadEnum = z.enum([
  '', 'Excelente', 'Buena', 'Media', 'Regular', 'Mala', 'N/A'
]);

export const NivelLuminosidadEnum = z.enum([
  '', 'Malo', 'Regular', 'Promedio', 'Buena', 'Excelente', 'N/A'
]);

export const OrientacionEnum = z.enum([
  '', 'Norte', 'Sur', 'Este', 'Oeste', 'Noreste', 'Sudeste', 'Noroeste', 'Sudoeste', 'N/A'
]);

export const TipoConstruccionEnum = z.enum([
  '', 'Ladrillo', 'Metalica', 'Madera', 'Bloques de hormigon', 'N/A'
]);

export const TipoTechoEnum = z.enum([
  '', 'N/A', 'Losa H°A°', 'Losa ceramica', 'Tejas s/ estr. Madera',
  'Pizarra s/ estr. Madera', 'Chapa s/ estr. Madera', 'Chapa s/ estr. Metalica'
]);

export const EstacionamientoEnum = z.enum([
  '', 'Garaje cubierto', 'Garaje semicubierto', 'Garaje descubierto', 'N/A'
]);

export const SiNoNAEnum = z.enum(['', 'Si', 'No', 'N/A']);

export const ServicioNivelEnum = z.enum([
  'Central', 'Individual', 'Inexistente', 'N/A'
]);
...
export const TipologiaEdiliciaEnum = z.enum([
  '', 'Construccion en altura', 'Construccion de media altura',
...
export const CalidadPredomEnum = z.enum([
  '', 'Excelente', 'Muy Buena', 'Buena', 'Media', 'Economica', 'Precaria'
]);

export const PrevalenciaEnum = z.enum([
  '', 'En todo el entorno', 'Sobre arterias principales', 'Ocasional', 'No relevante o inexistente'
]);

export const NivelSocioEnum = z.enum([
...
export const ConstruidoPctEnum = z.enum([
  '', 'Mas del 75%', 'Entre el 75% y el 25%', 'Menos del 25%'
]);
...
export const DemandaOfertaEnum = z.enum([
  '', 'Exceso de Oferta', 'Falta de Oferta', 'Relacion Oferta/Demanda Equilibrada'
]);

export const TiempoComercializacionEnum = z.enum([
  '', 'Menos de 3 meses', 'Entre 3 y 6 meses', 'Mas de 6 meses'
]);

export const CambiosUsoEnum = z.enum(['', 'Probable', 'Improbable', 'En Proceso']);

export const FacilidadesEstacionamientoEnum = z.enum([
  '', 'Garage Propio', 'Garajes privados', 'En la via publica'
]);
...
export const PESOS = {
  CASA: {
    vars: [
      ['Calidad de ubicacion', 0.30],
      ['Cantidad de habitaciones', 0.20],
      ['Estado de mantenimiento', 0.20],
      ['Antiguedad', 0.15],
      ['Comodidades', 0.10],
  ...
  DEPTO: {
    vars: [
      ['Calidad de ubicacion (barrio)', 0.30],
      ['Cantidad de habitaciones', 0.20],
      ['Ubicacion piso', 0.15],
      ['Antiguedad', 0.15],
  ...
  LOCAL: {
    vars: [
      ['Calidad de ubicacion', 0.35],
      ['Frente / vidriera', 0.20],
      ['Superficie y forma', 0.15],
      ['Instalaciones', 0.12],
      ['Estado de mantenimiento', 0.10],
...
export const AMBIENTE_IDS = [
  'f_ambCocina', 'f_ambDormitorios', 'f_ambTerraza', 'f_ambComedor',
  'f_ambSuite', 'f_ambPatio', 'f_ambCocinaComedor', 'f_ambSuiteVestidor',
  'f_ambBalcon', 'f_ambLiving', 'f_ambDormitVestidor', 'f_ambLavadero',
  'f_ambLivingComedor', 'f_ambBanoServicio', 'f_ambCuartoGuardado',
  'f_ambEscritorio', 'f_ambBano', 'f_ambGarage'
] as const;
...
export const ValuacionConValidacionesSchema = ValuacionInputSchema.refine(
  (data) => {
    return (data.f_supTerreno && data.f_supTerreno > 0) ||
           (data.f_supConstruida && data.f_supConstruida > 0);
  },
  { message: 'Se requiere al menos Superficie Terreno o Superficie Construida', path: ['f_supTerreno'] }
).refine(
  (data) => {
