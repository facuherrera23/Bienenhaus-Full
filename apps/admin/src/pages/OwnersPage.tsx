...
import { Download, Plus, Search, Trash2, X, Filter, User, Building2 } from 'lucide-preact';
...
import { OWNER_TYPE_LABEL, OWNER_PREFERRED_CONTACT_LABEL } from '@/types/owners';
...
export function OwnersPage() {
  ...
  const handleExport = () => {
    ...
    const header = [
      'Nombre', 'Tipo', 'Email', 'Teléfono', 'DNI/CUIT', 'Dirección',
      'Contacto preferido', 'Propiedades', 'Creado', 'Actualizado'
    ];
    ...
    <div className="page">
      <div className="page-head">
        <div>
          ...
          <p className="page-subtitle">Gestioná la cartera de propietarios de la inmobiliaria.</p>
        </div>
        <div style="display:flex; gap:8px; align-items:center;">
          {!showTrash && (
            <button type="button" className="btn btn--secondary" onClick={handleExport} disabled={owners.length === 0}>
        ...
        <div className="bulk-bar">
          ...
          <div className="bulk-actions">
            <button
              ...
              onClick={() => {
                if (window.confirm(`¿Enviar a papelera ${selectedIds.size} propietario${selectedIds.size === 1 ? '' : 's'}?`)) {
          ...
          <div className="filter-row">
            <div className="filter-group">
              ...
              <select
                className="select"
                value={typeFilter}
                onChange={(e) => setTypeFilter((e.currentTarget as HTMLSelectElement).value as 'todos' | OwnerType)}
              >
                <option value="todos">Todos</option>
                <option value="persona_fisica">{OWNER_TYPE_LABEL.persona_fisica}</option>
                <option value="persona_juridica">{OWNER_TYPE_LABEL.persona_juridica}</option>
              </select>
            </div>
            <div className="filter-group">
              ...
              <select
                className="select"
                value={contactFilter}
                onChange={(e) => setContactFilter((e.currentTarget as HTMLSelectElement).value as 'todos' | OwnerPreferredContact)}
              >
                <option value="todos">Todos</option>
                <option value="whatsapp">{OWNER_PREFERRED_CONTACT_LABEL.whatsapp}</option>
            ...
            <div className="filter-group">
              ...
              <select
                ...
                onChange={(e) => setHasPropertiesFilter((e.currentTarget as HTMLSelectElement).value as 'todos' | 'si' | 'no')}
              >
      ...
      {(isPending || deletedPending) && <div className="card placeholder-card">Cargando propietarios…</div>}
      {isError && <div className="card placeholder-card">No se pudieron cargar los propietarios.</div>}
        ...
        <>
          {filtered.length === 0 && (
            <div className="card placeholder-card">
              ...
              <p>{search ? 'No se encontraron coincidencias.' : 'Comenzá agregando el primer propietario.'}</p>
              <Link href="/propietarios/nuevo" className="btn btn--primary">
                <Plus size={16} /> Nuevo propietario
              </Link>
            </div>
                ...
                <tbody>
                  {deletedOwners.map((owner) => (
                    <tr key={owner.id}>
                      <td>
                        <div className="cell-property">
                          ...
                          <div>
                            ...
                            <span className="muted">{OWNER_TYPE_LABEL[owner.owner_type]}</span>
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td className="muted">{formatDate(owner.deleted_at ?? '')}</td>
                      <td>
                        <div className="row-actions">
                          <button
                            className="icon-btn"
                            title="Restaurar"
                            onClick={() => handleRestore(owner.id, owner.full_name)}
                          >
                            <Building2 size={14} />
                          </button>
                          <button
                            ...
                            onClick={() => handlePermanentDelete(owner.id, owner.full_name)}
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
