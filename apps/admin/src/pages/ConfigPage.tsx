...
export function ConfigPage() {
  const usersQ = useQuery<AdminUserRow[]>({ queryKey: ['admin-users'], queryFn: fetchAdminUsers });
  ...
  const isAdmin = myRow?.role === 'super_admin' || myRow?.role === 'admin';
  ...
  const roleMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({ type: 'error', title: 'No se pudo actualizar el rol', description: (err as Error).message });
    },
  });
  ...
  const activeMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({ type: 'error', title: 'No se pudo actualizar el estado', description: (err as Error).message });
    },
  });
  ...
  const resetMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({ type: 'error', title: 'No se pudo generar el enlace', description: (err as Error).message });
    },
  });
  ...
  const removeMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({ type: 'error', title: 'No se pudo eliminar', description: (err as Error).message });
    },
  });
                ...
                <tr key={u.id}>
                  ...
                  <td>
                    <select
                      ...
                      disabled={!isAdmin}
                      onChange={(e) => {
                        const role = (e.currentTarget as HTMLSelectElement).value as AdminRole;
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) =>
                        activeMutation.mutate({ id: u.id, is_active: (e.currentTarget as HTMLInputElement).checked })
                      }
                    />
                  </td>
                    ...
                    {isAdmin && (
                      <div className="row-actions">
                        ...
                        <button
                          ...
                          disabled={u.id === myId}
                          onClick={() => {
                            if (window.confirm(`¿Eliminar el acceso de ${u.email}?`)) {
      ...
      <section className="card">
        <div className="site-section-head">
          <div>
            ...
            <p>
              Identificación de la app y valores por defecto para publicar. La conexión y la cola se
              administran en la sección «Mercado Libre».
            </p>
          </div>
          <button className="btn btn--primary" onClick={saveMlConfig} disabled={savingMl || (!dirtyAppId && !dirtyDefaults)}>
        ...
        <div className="form-grid">
          ...
          <label className="field">
            ...
            <input
              ...
              onInput={(e) =>
                setDefaultsDraft((d) => ({ ...d, category_id: (e.currentTarget as HTMLInputElement).value }))
              }
            />
          </label>
          <label className="field">
            ...
            <select
              ...
              onChange={(e) =>
                setDefaultsDraft((d) => ({ ...d, listing_type_id: (e.currentTarget as HTMLSelectElement).value }))
              }
            >
          ...
          <label className="field">
            ...
            <select
              ...
              onChange={(e) =>
                setDefaultsDraft((d) => ({ ...d, condition: (e.currentTarget as HTMLSelectElement).value }))
              }
            >
      ...
      {inviteOpen && (
        <Modal title="Invitar usuario" onClose={() => setInviteOpen(false)}>
          <div className="form-grid">
            <label className="field">
              ...
              <input
                ...
                onInput={(e) => setInviteForm((f) => ({ ...f, full_name: (e.currentTarget as HTMLInputElement).value }))}
              />
            </label>
            <label className="field">
              <span>Email</span>
              <input
                ...
                onInput={(e) => setInviteForm((f) => ({ ...f, email: (e.currentTarget as HTMLInputElement).value }))}
              />
            </label>
            <label className="field">
              <span>Rol</span>
              <select
                ...
                onChange={(e) =>
                  setInviteForm((f) => ({ ...f, role: (e.currentTarget as HTMLSelectElement).value as AdminRole }))
                }
              >
                {(Object.keys(ADMIN_ROLE_LABEL) as AdminRole[]).map((r) => (
                  <option key={r} value={r}>
          ...
          <div className="modal-actions">
            ...
            >
              {inviteMutation.isPending ? <Loader2 size={15} className="spin" /> : <UserPlus size={15} />}
              Invitar
            </button>
          </div>
        </Modal>
      ...
      {linkModal && (
        <Modal title={linkModal.title} onClose={() => setLinkModal(null)}>
          ...
          <div className="link-box">
            ...
            <button type="button" className="icon-btn" title="Copiar enlace" onClick={copyLink}>
              <Copy size={14} />
            </button>
          </div>
          <div className="modal-actions">
            <button type="button" className="btn btn--primary" onClick={() => setLinkModal(null)}>
              Listo
            </button>
          </div>
        </Modal>
