...
import { Plus, Filter, Search, FileText, Download, MessageSquare, Trash2 } from 'lucide-preact';
...
export function ReportsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'todos' | 'draft' | 'sent' | 'delivered' | 'read' | 'failed'>('todos');
  ...
  const handleGenerateReport = async (reportData: ReportFormValues) => {
    try {
      ...
      pushToast({ type: 'success', title: 'Reporte generado', description: reportData.title ?? undefined });
  ...
  const tabs = [
    ...
    { id: 'drafts', label: 'Borradores', count: allReports.filter(r => r.status === 'draft').length },
    { id: 'sent', label: 'Enviados', count: allReports.filter(r => r.status !== 'draft').length },
  ];
    ...
    <div className="page reports-page">
      <div className="page-head">
        <div>
          ...
          <p className="page-subtitle">Generá y enviá reportes automáticos a propietarios.</p>
        </div>
        <button
          type="button"
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="card placeholder-card">
          ...
          <h3>Sin reportes</h3>
          <p>{search ? 'No se encontraron coincidencias.' : 'No se han generado reportes.'}</p>
            ...
            <tbody>
              {filtered.map((report) => (
                <tr key={report.id}>
                  ...
                  <td>
                    <span className="badge badge--neutral">{REPORT_TYPE_LABEL[report.report_type]}</span>
                  </td>
                  <td>{report.property_title ?? '—'}</td>
                  <td>{report.owner_name ?? '—'}</td>
                  <td>
                    <span className={`badge badge--${report.status === 'draft' ? 'neutral' : report.status === 'sent' ? 'info' : report.status === 'delivered' ? 'info' : report.status === 'read' ? 'success' : 'danger'}`}>
                      {report.status}
                    </span>
                  </td>
                  <td>{formatDateShort(report.generated_at)}</td>
                  <td>{report.sent_at ? formatDateShort(report.sent_at) : '—'}</td>
                  <td>
                    <div className="row-actions">
                      ...
                      <button
                        ...
                        title="Eliminar"
                        onClick={() => handleDelete(report.id, report.title || 'Sin título')}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
