...
function StatusBadge({ status }: { status: MlSyncStatus }) {
  return <span className={`badge badge--${ML_SYNC_STATUS_TONE[status]}`}>{ML_SYNC_STATUS_LABEL[status]}</span>;
}
...
export function MercadoLibrePage() {
  ...
  const [defaultsDraft, setDefaultsDraft] = useState({ category_id: '', listing_type_id: 'gold_pro', condition: 'used' });
  ...
  type TemplateTrigger = 'new_question' | 'new_order' | 'order_paid' | 'order_shipped' | 'order_delivered';
  const [showAutoReplyModal, setShowAutoReplyModal] = useState(false);
  const [editingTemplate, setEditingTemplate] = useState<import('../lib/ml').MlAutoReplyTemplate | null>(null);
  ...
  const disconnectMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({ type: 'error', title: 'Error al desconectar', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
    },
  });
  ...
  const toggleEnabled = async (checked: boolean) => {
    try {
      ...
      pushToast({ type: 'success', title: checked ? 'Integracion activada' : 'Integracion desactivada' });
      await queryClient.invalidateQueries({ queryKey: ['ml-overview'] });
    } catch (err) {
      pushToast({ type: 'error', title: 'Error al cambiar estado', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
    }
  };
  ...
  const saveAppId = async () => {
    ...
    try {
      ...
      pushToast({ type: 'error', title: 'Error al guardar ID', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
    } finally {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      pushToast({ type: 'error', title: 'No se pudo responder', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
    } finally {
      setReplying(false);
    }
  };
  ...
  const handleTemplateSave = async () => {
    if (!templateForm.name.trim() || !templateForm.message.trim()) {
      pushToast({ type: 'warning', title: 'Faltan datos', description: 'Completa nombre y mensaje.' });
      return;
    }
    ...
    try {
      ...
      pushToast({ type: 'success', title: editingTemplate ? 'Plantilla actualizada' : 'Plantilla creada' });
      setShowAutoReplyModal(false);
      setEditingTemplate(null);
      await queryClient.invalidateQueries({ queryKey: ['ml-auto-reply'] });
    } catch (err) {
      pushToast({ type: 'error', title: 'Error al guardar plantilla', description: err instanceof Error ? err.message : 'Intenta de nuevo.' });
    } finally {
  ...
  const metricsSection = showMetrics && connected && metricsQ.data ? (
    <section className="card">
      <div className="site-section-head">
        ...
        <button className="btn btn--ghost btn--sm" onClick={() => setShowMetrics(false)}>
          <X size={16} /> Cerrar
        </button>
      </div>
      ...
      <div className="site-section-head">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            ...
            onClick={() => { setEditingTemplate(null); setTemplateForm({ name: '', trigger: 'new_question', message: '', is_active: true }); setShowAutoReplyModal(true); }}
          >
            <Plus size={14} /> Nueva plantilla
          </button>
          <button className="btn btn--ghost btn--sm" onClick={() => setShowAutoReply(false)}>
            <X size={16} /> Cerrar
          </button>
        </div>
      </div>
    ...
    <div className="modal-backdrop" onClick={() => setSelectedQuestion(null)}>
      <div className="modal-card" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          ...
          <button className="icon-btn" onClick={() => setSelectedQuestion(null)}><X size={20} /></button>
        </div>
        <div className="modal-body">
          <p className="muted" style={{ marginBottom: '8px' }}>
            {selectedQuestion.from_user_nickname ?? 'Comprador'} preguntaba:
          </p>
          <blockquote style={{ margin: '0 0 16px', padding: '12px', background: 'var(--bh-surface-2)', borderRadius: '8px' }}>
        ...
        <div className="modal-actions">
          <button className="btn btn--ghost" onClick={() => setSelectedQuestion(null)}>Cancelar</button>
          <button className="btn btn--primary" onClick={handleReply} disabled={!replyText.trim() || replying}>
            {replying ? <Loader2 size={14} className="spin" /> : <Send size={14} />} Enviar respuesta
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="modal-backdrop" onClick={closeTemplateModal}>
      <div className="modal-card modal--medium" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          ...
          <button className="icon-btn" onClick={closeTemplateModal}><X size={20} /></button>
        </div>
        <div className="modal-body">
          <div style={{ display: 'grid', gap: '16px' }}>
            <label className="field">
              ...
              <input
                ...
                onInput={e => setTemplateForm({ ...templateForm, name: (e.currentTarget as HTMLInputElement).value })}
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={e => setTemplateForm({ ...templateForm, trigger: e.currentTarget.value as typeof templateForm.trigger })}
              >
            ...
            <label className="field">
              ...
              <textarea
                ...
                onInput={e => setTemplateForm({ ...templateForm, message: (e.currentTarget as HTMLTextAreaElement).value })}
            ...
            <label className="switch-row">
              <div>
                ...
                <span className="muted">La plantilla se usa para respuestas automaticas.</span>
              </div>
              <input
                type="checkbox"
                className="switch"
                checked={templateForm.is_active}
                onChange={e => setTemplateForm({ ...templateForm, is_active: (e.currentTarget as HTMLInputElement).checked })}
              />
            </label>
          </div>
        </div>
        <div className="modal-actions">
          <button className="btn btn--ghost" onClick={closeTemplateModal}>Cancelar</button>
          <button className="btn btn--primary" onClick={handleTemplateSave} disabled={savingTemplate}>
            {savingTemplate ? <Loader2 size={14} className="spin" /> : <CheckCircle2 size={14} />}
  ...
  return (
    <div className="page">
      ...
      {overviewQ.isError && <div className="card placeholder-card">No se pudo cargar la integracion.</div>}

      {!overviewQ.isPending && !overviewQ.isError && (
        <>
          {connected && (
            <div className="ml-section-toggles" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
              <button className={`btn btn--sm ${showMetrics ? 'btn--primary' : 'btn--ghost'}`} onClick={() => setShowMetrics(v => !v)}>
                <BarChart2 size={14} /> {showMetrics ? 'Ocultar' : 'Ver'} metricas
              </button>
              <button className={`btn btn--sm ${showQuestions ? 'btn--primary' : 'btn--ghost'}`} onClick={() => setShowQuestions(v => !v)}>
                <MessageSquare size={14} /> Preguntas {questionsQ.data ? `(${questionsQ.data.filter(q => q.status === 'unanswered').length})` : ''}
              </button>
              <button className={`btn btn--sm ${showAutoReply ? 'btn--primary' : 'btn--ghost'}`} onClick={() => setShowAutoReply(v => !v)}>
            ...
            <section className="card">
              <div className="site-section-head">
                <div>
                  ...
                  <p>El secret de los tokens nunca se guarda en la base: se cifra con AES-256-GCM.</p>
                </div>
              </div>
              ...
              <div className="switch-row">
                <div>
                  ...
                  <span className="muted">Habilita la cola de sincronizacion y la auto-publicacion.</span>
                </div>
                <input
                  type="checkbox"
                  className="switch"
              ...
              {connection ? (
                <div className="ml-connection">
                  <div className="ml-connection-identity">
                    ...
                    <div>
                      <strong>{connection.nickname ?? 'Cuenta Mercado Libre'}</strong>
                      <span className="muted">{connection.email ?? connection.site_id}</span>
                    </div>
                  </div>
                  ...
                  <div className="ml-defaults">
                    ...
                    <div className="defaults-grid">
                      <label className="field">
                        ...
                        <div className="typeahead" onMouseLeave={() => setShowCategoryDropdown(false)}>
                          <Search size={16} className="typeahead-icon" />
                          <input
                            type="text"
                            placeholder="Buscar categoria... (ej: MLA1459)"
                            value={categorySearch || defaultsDraft.category_id}
                            onInput={(e) => {
                              const val = (e.currentTarget as HTMLInputElement).value;
                              setCategorySearch(val);
                              setShowCategoryDropdown(true);
                              setDefaultsDraft({ ...defaultsDraft, category_id: val });
                            }}
                            onFocus={() => setShowCategoryDropdown(true)}
                            onBlur={() => setTimeout(() => setShowCategoryDropdown(false), 150)}
                          />
                          {showCategoryDropdown && categorySearch && categoriesQ.data && (
                            <ul className="typeahead-dropdown">
                              {categoriesQ.data
                                ...
                                .map((c: MlCategory) => (
                                  <li
                                    key={c.id}
                                    onClick={() => {
                                      setDefaultsDraft({ ...defaultsDraft, category_id: c.id });
                                      setCategorySearch(`${c.id} - ${c.name}`);
                              ...
                              {categoriesQ.data.filter((c: MlCategory) =>
                                c.name.toLowerCase().includes(categorySearch.toLowerCase()) ||
                                c.id.toLowerCase().includes(categorySearch.toLowerCase())
                              ).length === 0 && (
                          ...
                          {categorySearch && defaultsDraft.category_id && (
                            <button
                              type="button"
                              className="typeahead-clear"
                              onClick={() => {
                                setDefaultsDraft({ ...defaultsDraft, category_id: '' });
                                setCategorySearch('');
                              }}
                              title="Limpiar"
                            >
                      ...
                      <label className="field">
                        ...
                        <div className="select-with-desc">
                          <select
                            ...
                            onChange={(e) => setDefaultsDraft({ ...defaultsDraft, listing_type_id: (e.currentTarget as HTMLSelectElement).value })}
                          >
                            {(listingTypesQ.data ?? [
                              { id: 'free', name: 'Gratuita' },
                              { id: 'gold_special', name: 'Clasica' },
                              { id: 'gold_pro', name: 'Premium' },
                              { id: 'gold_premium', name: 'Premium Plus' },
                            ]).map((lt: MlListingType | { id: string; name: string }) => (
                              <option key={lt.id} value={lt.id}>{lt.name} ({lt.id})</option>
                            ))}
                          </select>
                          <p className="select-desc">{LISTING_TYPE_DESCRIPTIONS[defaultsDraft.listing_type_id]}</p>
                        </div>
                      </label>
                      <label className="field">
                        <span>Condicion</span>
                        <select
                          ...
                          onChange={(e) => setDefaultsDraft({ ...defaultsDraft, condition: (e.currentTarget as HTMLSelectElement).value })}
                        >
                    ...
                    <div className="ml-connection-actions">
                      {JSON.stringify(defaultsDraft) !== JSON.stringify(settingsQ.data?.defaults ?? {}) && (
                        <button className="btn btn--secondary btn--sm" onClick={saveDefaults} disabled={savingDefaults}>
                          {savingDefaults ? <Loader2 size={14} className="spin" /> : <CheckCircle2 size={14} />}
                ...
                <div className="ml-connect">
                  ...
                  <div className="ml-redirect">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        pushToast({ type: 'info', title: 'Redirect URI copiada' });
                      }}
                    >
                      <Copy size={14} />
                    </button>
                  ...
                  <div className="ml-connection-actions">
                    {appIdDraft.trim() !== (settingsQ.data?.app_id ?? '') && (
                      <button className="btn btn--secondary btn--sm" onClick={saveAppId} disabled={savingAppId}>
                        {savingAppId ? <Loader2 size={14} className="spin" /> : <CheckCircle2 size={14} />}
                        Guardar ID
                      </button>
                    )}
            ...
            <table className="table">
              ...
              <tbody>
                {queueRows.map((q) => (
                  <tr key={q.id}>
                    <td>
                      <strong>{q.property_title ?? 'Propiedad eliminada'}</strong>
                      {q.property_code !== null && <span className="muted"> #{String(q.property_code).padStart(4, '0')}</span>}
                    </td>
                    ...
                    <td className="muted cell-error" title={q.last_error ?? undefined}>
                      {q.last_error ?? '-'}
                    </td>
                  </tr>
                ))}
                ...
                {(metaQ.data ?? []).map((m) => (
                  <tr key={m.property_id}>
                    <td>
                      ...
                      {m.property_code !== null && <span className="muted"> #{String(m.property_code).padStart(4, '0')}</span>}
                    </td>
                    <td className="num">{m.ml_item_id ?? '-'}</td>
                    <td className="cap">{m.status ?? '-'}</td>
                    <td className="num">{m.price !== null ? `USD ${m.price.toLocaleString('es-AR')}` : '-'}</td>
                    <td className="muted">{formatDate(m.last_sync_at)}</td>
                    <td>
                      {m.permalink ? (
                        <a href={m.permalink} target="_blank" rel="noreferrer" className="btn btn--sm btn--secondary">
