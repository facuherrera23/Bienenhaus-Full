...
function AuditLogRow({ log, onClick }: { log: AuditLogEntry; onClick: () => void }) {
  ...
  return (
    <tr key={log.id} onClick={onClick} style={{ cursor: 'pointer' }}>
      ...
      <td>
        {statusStyle && (
          <span className="status-badge" style={{ borderColor: statusStyle.color, color: statusStyle.color }}>
...
export function AuditLogPage() {
  ...
  const exportLogs = async () => {
    ...
    pushToast({ type: 'success', title: 'Exportado', description: `${logs.length} logs exportados` });
  };
  ...
  return (
    <div className="page">
      ...
      <div className={`audit-filters${showFilters ? ' open' : ''}`}>
        <button className="audit-filters-toggle" onClick={() => setShowFilters(!showFilters)}>
          <Filter size={16} /> Filtros {hasActiveFilters && <span className="badge badge--info">{Object.values(filters).filter(v => v).length}</span>}
        </button>
        ...
        {showFilters && (
          <div className="audit-filters-panel">
            <div className="filter-row">
              ...
              <div className="field">
                <label>Acción</label>
                <select className="select" value={filters.action} onChange={e => handleFilterChange('action', e.currentTarget.value)}>
              ...
              <div className="field">
                <label>Entidad</label>
                <select className="select" value={filters.entity_type} onChange={e => handleFilterChange('entity_type', e.currentTarget.value)}>
                  <option value="">Todas</option>
              ...
              <div className="field">
                <label>Estado</label>
                <select className="select" value={filters.status} onChange={e => handleFilterChange('status', e.currentTarget.value)}>
            ...
            <div className="filter-row">
              <div className="field">
                <label>Desde</label>
                <input type="date" className="select" value={filters.from_date} onChange={e => handleFilterChange('from_date', e.currentTarget.value)} />
              </div>
              <div className="field">
                <label>Hasta</label>
                <input type="date" className="select" value={filters.to_date} onChange={e => handleFilterChange('to_date', e.currentTarget.value)} />
              </div>
            </div>
          </div>
        )}
        ...
        <div className="modal-backdrop" onClick={closeDetail}>
          <div className="modal-card modal--large" onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              ...
              <button className="icon-btn" onClick={closeDetail}><X size={20} /></button>
            </div>
            <div className="modal-body">
              <div className="audit-detail-grid">
                <div className="audit-detail-field">
                  <label>Acción</label>
                  <span><strong>{ACTION_LABELS[selectedLog.action] ?? selectedLog.action}</strong></span>
                </div>
                <div className="audit-detail-field">
                  <label>Entidad</label>
                ...
                <div className="audit-detail-field">
                  <label>Fecha</label>
                  <span>{new Date(selectedLog.created_at).toLocaleString('es-AR')}</span>
                </div>
                <div className="audit-detail-field">
                  <label>Estado</label>
                  <span>
                ...
                <div className="audit-detail-field audit-detail-wide">
                  <label>Actor</label>
                  <div>
                    <div><strong>Email:</strong> {selectedLog.actor_email ?? 'Sistema'}</div>
                    <div><strong>Rol:</strong> {selectedLog.actor_role ?? '—'}</div>
                    <div><strong>IP:</strong> {selectedLog.actor_ip ?? '—'}</div>
                  </div>
                </div>
                <div className="audit-detail-field audit-detail-wide">
                ...
                {selectedLog.changed_fields && selectedLog.changed_fields.length > 0 && (
                  <div className="audit-detail-field audit-detail-wide">
                    ...
                    <div className="changed-fields">
                      {selectedLog.changed_fields.map(f => <span key={f} className="changed-field-tag">{f}</span>)}
                    </div>
                  </div>
                )}
                ...
                {selectedLog.error_message && (
                  <div className="audit-detail-field audit-detail-wide">
                    ...
                    <div className="error-message">
                      <AlertCircle size={14} style={{ color: 'var(--bh-danger)' }} />
                      {selectedLog.error_message}
                    </div>
                  </div>
                )}
                ...
                <div className="audit-detail-field audit-detail-wide">
                  ...
                  <pre className="json-preview">{JSON.stringify(selectedLog.old_values, null, 2) || '—'}</pre>
                </div>

                <div className="audit-detail-field audit-detail-wide">
                  <label>Valores nuevos</label>
                  <pre className="json-preview">{JSON.stringify(selectedLog.new_values, null, 2) || '—'}</pre>
                </div>

                {selectedLog.metadata && Object.keys(selectedLog.metadata).length > 0 && (
                  <div className="audit-detail-field audit-detail-wide">
                    <label>Metadatos</label>
                    <pre className="json-preview">{JSON.stringify(selectedLog.metadata, null, 2)}</pre>
                  </div>
                )}
              </div>
            </div>
