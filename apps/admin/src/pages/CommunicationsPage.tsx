...
import { Filter, MessageSquare, Plus, Search, X } from 'lucide-preact';
...
export function CommunicationsPage() {
    ...
    const handleSubmit = async (data: CommunicationFormValues) => {
        try {
            ...
            pushToast({
                ...
                description: payload.subject || 'Sin asunto',
            });
    ...
    const handleSend = async (comm: CommunicationRow) => {
        try {
            ...
            pushToast({
                type: 'success',
                title: 'Enviado',
                description: comm.subject ?? undefined,
            });
            queryClient.invalidateQueries({ queryKey: ['owner-communications'] });
        } catch {
    ...
    const tabs: { id: 'all' | 'drafts' | 'sent'; label: string; count: number }[] = [
        { id: 'all', label: 'Todas', count: allCommunications.length },
        {
            id: 'drafts',
            label: 'Borradores',
            count: allCommunications.filter((c) => c.status === 'draft').length,
        },
        {
            id: 'sent',
            label: 'Enviadas',
        ...
        <div className="page communications-page">
            ...
            {showNewComm && (
                <div className="card form-card">
                    <form
                        onSubmit={commMethods.handleSubmit(
                            editingComm ? handleSubmit : handleSubmit,
                        )}
                        className="comm-form"
                    >
                        ...
                        <div className="field full-width">
                            ...
                            <textarea
                                id="content"
                                rows={4}
                                placeholder="Mensaje, notas, detalles..."
                                {...commMethods.register('content')}
                            />
                        </div>
