      ...
      <div className="page-head">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {totalCount > 0 && <span className="muted"> · Restaurar o eliminar permanentemente</span>}
          </p>
        </div>
        {totalCount > 0 && (
          <button className="btn btn--ghost" onClick={() => items.forEach((i) => handleRestore(tabToType(activeTab), i.id, ''))} disabled={pending}>
      ...
      {!pending && items.length > 0 && (
        <div className="card table-card">
          <table className="table">
            ...
            <tbody>
              {items.map((item) => renderRow(item, tabToType(activeTab)))}
            </tbody>
          </table>
        </div>
      )}
