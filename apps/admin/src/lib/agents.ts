...
export type AgentApiRow = Database['public']['Tables']['agents']['Row'] & {
  leads: { count: number }[];
  specialties: unknown;
  social: unknown;
  permissions?: unknown;
...
export function toRow(a: AgentApiRow): AgentRow {
  ...
  const permissions = a.permissions && typeof a.permissions === 'object'
    ? a.permissions as unknown as AgentPermissions
    : DEFAULT_PERMISSIONS;
const commission = a.commission && typeof a.commission === 'object'
     ? a.commission as unknown as AgentCommission
     : DEFAULT_COMMISSION;
const schedule = Array.isArray(a.schedule)
     ? (a.schedule as unknown as AgentSchedule[])
     : DEFAULT_SCHEDULE;
...
export async function fetchAgents(): Promise<AgentRow[]> {
  ...
  return ((data ?? []) as unknown as AgentApiRow[]).map(toRow);
}
...
export async function fetchAgent(id: string): Promise<AgentRow> {
  ...
  return toRow(data as unknown as AgentApiRow);
}
...
export async function getAgentPermissions(id: string): Promise<AgentPermissions> {
  ...
  if (error) throw new Error(error.message);
  return (data?.permissions as unknown as AgentPermissions) ?? DEFAULT_PERMISSIONS;
}
...
export async function updateAgentSchedule(id: string, schedule: AgentSchedule[]): Promise<void> {
  const { error } = await supabase.from('agents').update({ schedule: schedule as unknown as Json }).eq('id', id);
  if (error) throw new Error(error.message);
}
