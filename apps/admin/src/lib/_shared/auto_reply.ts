 ...
 * Versión admin (Node/Vite) — para unit tests.
 * La versión Deno edge está en supabase/functions/_shared/auto_reply.ts.
 *
 * NOTA: esta versión importa @supabase/supabase-js (sin prefijo npm:)
 * y los helpers de ML son stubs que las tests mockean vía vi.mock.
 */

import type { SupabaseClient } from '@supabase/supabase-js';

// Stubs — las tests mockean fetch y getAccessToken vía vi.mock.
// En edge, estos vienen de ./ml.ts (Deno). En admin no se ejecutan de verdad.
...
async function fetchMlWithRetry(fn: () => Promise<Response>): Promise<Response> {
    return fn();
}
