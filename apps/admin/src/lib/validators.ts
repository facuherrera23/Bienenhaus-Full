...
export const propertyCreateSchema = propertyFormSchema
    .refine((data) => data.listing_type !== 'venta' || (data.price != null && data.price > 0), {
        message: 'El precio es obligatorio para propiedades en venta',
        path: ['price'],
    })
    .refine((data) => data.listing_type !== 'alquiler' || (data.price != null && data.price > 0), {
        message: 'El precio es obligatorio para propiedades en alquiler',
        path: ['price'],
    })
    .refine(
...
export const newsletterSourceSchema = z.enum(['landing_footer', 'manual', 'otro']);

export const newsletterStatusSchema = z.enum(['active', 'unsubscribed', 'bounced', 'complained']);
...
export const adminRoleSchema = z.enum(['super_admin', 'admin', 'staff', 'viewer']);
