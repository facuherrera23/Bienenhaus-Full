...
export type ReportType =
    'price_analysis' | 'visit_summary' | 'market_update' | 'weekly' | 'monthly' | 'custom';
...
export type CommunicationRow = z.infer<typeof communicationSchema> & {
    id: string;
    sent_at: string | null;
    sent_by: string | null;
...
export const reportsFiltersSchema = z.object({
    ...
    owner_id: z.string().uuid().optional(),
    report_type: z
        .enum(['price_analysis', 'visit_summary', 'market_update', 'weekly', 'monthly', 'custom'])
        .optional(),
    status: z.enum(['draft', 'sent', 'delivered', 'read', 'failed']).optional(),
    page: z.number().int().positive().default(1),
    pageSize: z.number().int().positive().max(100).default(20),
