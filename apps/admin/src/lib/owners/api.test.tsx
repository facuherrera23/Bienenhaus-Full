  ...
  property_owners: [
    {
      ...
      role: 'Titular',
      properties: { title: 'Casa Centro', address: 'Av. 1', price: 150000, status: 'publicada' },
    },
  ],
};
...
describe('Owners API Functions', () => {
  ...
  describe('fetchOwnersPaginated', () => {
    ...
    it('filters by has_properties post-query', async () => {
      const ownerWithProps = { ...ownerRow, id: '2', property_owners: [{ property_id: 'p1', ownership_percentage: 100, is_primary_contact: true, role: '' }] };
      const builder = makeBuilder({ data: [ownerRow, ownerWithProps], error: null, count: 2 });
      from.mockImplementation(() => builder);
  ...
  describe('createOwner', () => {
    ...
    it('throws on error', async () => {
      ...
      await expect(
        createOwner({ full_name: 'A', email: 'a@a.com', owner_type: 'persona_fisica', preferred_contact: 'whatsapp' })
  ...
  describe('Price Analysis', () => {
    ...
    it('computes price_status as fair when listing equals market', async () => {
      ...
      expect(builder.insert).toHaveBeenCalledWith(expect.objectContaining({ price_status: 'fair' }));
    });
    ...
    it('marks price as way_above when listing is 30% over market', async () => {
      ...
      expect(builder.insert).toHaveBeenCalledWith(expect.objectContaining({ price_status: 'way_above' }));
    });
    ...
    it('recalculates price_status on update when prices change', async () => {
      ...
      expect(builder.update).toHaveBeenCalledWith(expect.objectContaining({ price_status: 'above' }));
    });
  });
