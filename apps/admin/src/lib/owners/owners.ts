...
interface OwnerApiRow extends OwnerDbRow {
  property_owners?: Array<{
    ...
    role: string;
    properties?: { title: string; address: string; price: number | null; status: string } | { title: string; address: string; price: number | null; status: string }[] | null;
  }>;
}
...
function embedFullName(v: { full_name: string } | { full_name: string }[] | null | undefined): string | null {
  if (!v) return null;
...
function toPropertyOwnerLinkRow(po: PropertyOwnerDbRow & { properties?: { title: string } | { title: string }[] | null; owners?: { full_name: string } | { full_name: string }[] | null }): PropertyOwnerLinkRow {
  const prop = Array.isArray(po.properties) ? po.properties[0] : po.properties;
...
function toPriceAnalysisRow(pa: PriceAnalysisApiRow): PriceAnalysisRow {
  ...
  return {
    ...
    comparable_properties: Array.isArray(comps) ? (comps as unknown as ComparableProperty[]) : [],
...
export async function fetchOwnersPaginated(filters: {
  ...
  sortOrder?: 'asc' | 'desc';
}): Promise<{ data: OwnerRow[]; count: number; page: number; pageSize: number; totalPages: number }> {
...
export async function createOwner(owner: Omit<OwnerFormValues, 'id'>): Promise<OwnerRow> {
  const { data, error } = await supabase.from('owners').insert(owner).select(OWNERS_SELECT).single();
  if (error) throw new Error(error.message);
...
export async function updateOwner(id: string, owner: Partial<OwnerFormValues>): Promise<OwnerRow> {
  const { data, error } = await supabase.from('owners').update(owner).eq('id', id).select(OWNERS_SELECT).single();
  if (error) throw new Error(error.message);
  return toOwnerRow(data as unknown as OwnerApiRow);
}
...
export async function fetchPropertyOwners(propertyId: string): Promise<PropertyOwnerLinkRow[]> {
  ...
  if (error) throw new Error(error.message);
  return ((data ?? []) as unknown as Array<PropertyOwnerDbRow & { properties?: { title: string }; owners?: { full_name: string } }>).map(toPropertyOwnerLinkRow);
}
...
export async function linkOwnerToProperty(link: PropertyOwnerLink): Promise<PropertyOwnerLinkRow> {
  const { data, error } = await supabase.from('property_owners').insert(link).select(PROPERTY_OWNERS_SELECT).single();
  if (error) throw new Error(error.message);
  return toPropertyOwnerLinkRow(data as unknown as PropertyOwnerDbRow & { properties?: { title: string }; owners?: { full_name: string } });
}
...
export async function updatePropertyOwnerLink(propertyId: string, ownerId: string, updates: Partial<PropertyOwnerLink>): Promise<PropertyOwnerLinkRow> {
  const { data, error } = await supabase
    .from('property_owners')
    .update(updates)
    .eq('property_id', propertyId)
...
export async function createPriceAnalysis(analysis: PriceAnalysisFormValues): Promise<PriceAnalysisRow> {
...
export async function updatePriceAnalysis(id: string, analysis: Partial<PriceAnalysisFormValues>): Promise<PriceAnalysisRow> {
...
export async function fetchActionPlans(filters?: {
  ...
  sortOrder?: 'asc' | 'desc';
}): Promise<{ data: ActionPlanRow[]; count: number; page: number; pageSize: number; totalPages: number }> {
  const page = filters?.page ?? 1;
  const pageSize = filters?.pageSize ?? 20;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
...
export async function fetchActionPlanById(id: string): Promise<ActionPlanDetail | null> {
  const [planRes, tasksRes] = await Promise.all([
    supabase.from('property_action_plans').select(ACTION_PLANS_SELECT).eq('id', id).maybeSingle(),
    supabase.from('action_plan_tasks').select(ACTION_PLAN_TASKS_SELECT).eq('plan_id', id).order('due_date'),
  ]);
  ...
  const tasks = ((tasksRes.data ?? []) as unknown as ActionPlanTaskApiRow[]).map(toActionPlanTaskRow);
...
export async function updateActionPlan(id: string, plan: Partial<ActionPlanFormValues>): Promise<ActionPlanRow> {
  const { data, error } = await supabase
    .from('property_action_plans')
    .update(plan)
    .eq('id', id)
...
export async function createActionPlanTask(task: ActionPlanTaskFormValues): Promise<ActionPlanTaskRow> {
  const { data, error } = await supabase
    .from('action_plan_tasks')
    .insert(task)
    .select(ACTION_PLAN_TASKS_SELECT)
...
export async function updateActionPlanTask(id: string, task: Partial<ActionPlanTaskFormValues>): Promise<ActionPlanTaskRow> {
  const payload = { ...task };
...
export async function fetchCommunications(filters?: {
  ...
  sortOrder?: 'asc' | 'desc';
}): Promise<{ data: CommunicationRow[]; count: number; page: number; pageSize: number; totalPages: number }> {
  const page = filters?.page ?? 1;
  const pageSize = filters?.pageSize ?? 20;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  ...
  let q = supabase
    ...
    .select(COMMUNICATIONS_SELECT, { count: 'exact' });
...
export async function createCommunication(comm: CommunicationFormValues): Promise<CommunicationRow> {
  const payload = {
    ...comm,
...
export async function createDraftCommunication(comm: CommunicationFormValues): Promise<CommunicationRow> {
  const { data, error } = await supabase
    .from('owner_communications')
...
export async function fetchReports(filters?: {
  ...
  sortOrder?: 'asc' | 'desc';
}): Promise<{ data: ReportRow[]; count: number; page: number; pageSize: number; totalPages: number }> {
  const page = filters?.page ?? 1;
  const pageSize = filters?.pageSize ?? 20;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
...
export async function createReport(report: ReportFormValues): Promise<ReportRow> {
  const { data, error } = await supabase
    .from('owner_reports')
    .insert({ ...report, generated_at: new Date().toISOString(), content_json: report.content_json as any })
    .select(REPORTS_SELECT)
    .single();
  if (error) throw new Error(error.message);
