...
export function useCreateRecurringVisit() {
  return useMutation({
    mutationFn: async ({ baseVisitId, rule }: { baseVisitId: string; rule: RecurrenceRule }) => {
...
export const VISIT_EXPORT_COLUMNS: ExportColumn<VisitRow>[] = [
  ...
  { key: 'starts_at', label: 'Inicio', format: (v) => v ? new Date(v).toLocaleString('es-AR') : '—' },
  { key: 'ends_at', label: 'Fin', format: (v) => v ? new Date(v).toLocaleString('es-AR') : '—' },
