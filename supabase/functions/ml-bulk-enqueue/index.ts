...
Deno.serve(async (req) => {
  ...
  const propertyIds = Array.isArray(payload.property_ids)
    ...
    : [];
  const operation = typeof payload.operation === 'string' ? payload.operation as MlOperation : null;
  ...
  const { data: settings } = await supabase
    ...
    .eq('key', 'ml_enabled')
    .maybeSingle();
  const mlEnabled = settings?.value ? Boolean((settings.value as { value?: unknown }).value) : false;
  if (!mlEnabled) {
