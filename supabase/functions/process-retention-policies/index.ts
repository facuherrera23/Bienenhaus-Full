    ...
    try {
        ...
        for (const policy of policies ?? []) {
            ...
            if (toNotify?.length) {
                ...
                console.log(`[retention] Notificar ${toNotify.length} items en ${policy.entity} próximos a auto-eliminación`);
                ...
                for (const item of toNotify) {
                    await supabase.from('notifications').insert({
                        ...
                        metadata: { entity: policy.entity, days_remaining: policy.notify_before_days },
                    });
                }
            }
            ...
            if (toDelete?.length) {
                console.log(`[retention] Auto-eliminando ${toDelete.length} items de ${policy.entity}`);
