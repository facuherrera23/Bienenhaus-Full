...
async function prepareImagesForML(
    ...
    const validImages = images.slice(0, 12).filter((i): i is { url: string; storage_path: string } => !!i.storage_path);
...
async function moveToDeadLetter(
    ...
    await supabase.from('ml_sync_queue').update({ status: 'failed', locked_by: null, locked_at: null, last_error: error }).eq('id', job.id);
...
async function runJob(
    ...
    if (operation === 'publish' && !defaults.category_id) return { ok: false, error: 'Falta configurar la categoría de Mercado Libre.' };
    if (operation === 'publish' && !defaults.listing_type_id) return { ok: false, error: 'Falta configurar el tipo de publicación de Mercado Libre.' };
    ...
    try {
        if (operation === 'publish') {
            ...
            const mlTitle = `${operationLabel} ${propertyType} ${roomsLabel} amb. ${location || 'Salta'}`.slice(0, 60);
            ...
            if (property.garages !== null && property.garages > 0) attributes.push({ id: 'PARKING_LOTS', value_name: String(property.garages) });
            ...
            const payload: MlItemPayload = {
                ...
                condition: ['new', 'used'].includes(defaults.condition) ? defaults.condition as 'new' | 'used' : 'not_specified',
            ...
            if (mlImageUrls.length === 0) {
                return { ok: false, error: 'Mercado Libre requiere al menos una imagen válida para esta publicación. Verifica que la propiedad tenga imágenes accesibles.' };
            }
            ...
            if (property.description) {
                ...
                if ('error' in descResult) logger.warn({ function: 'ml-sync', job_id: queueId, property_id: propertyId, operation: 'mlSetDescription', error: descResult.error });
            }
