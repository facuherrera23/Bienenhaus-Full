...
async function main() {
  ...
  console.log(`[fetch-data] Properties: ${properties.length}, Agents: ${agents.length}, Locations: ${locations.length}`);
  ...
  writeFileSync(
    ...
    JSON.stringify(mappedProperties, null, 2)
  );
  ...
  writeFileSync(
    ...
    JSON.stringify(mappedAgents, null, 2)
  );

  // Generate locations.json
  writeFileSync(
    join(OUT_DIR, 'locations.json'),
