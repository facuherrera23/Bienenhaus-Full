...
if (visit.property?.latitude && payload.agentLat && payload.agentLng) {
    const distance = haversine(
        ...
        payload.agentLat, payload.agentLng
    );
...
await supabase.from('qr_checkins').update({
    checked_in: true,
    checked_in_at: now,
...
const checkRecurringConflicts = async (rule: RecurrenceRule, baseVisit: VisitRow) => {
    ...
    for (const occ of occurrences) {
        const conflicts = await checkConflicts({ ...baseVisit, starts_at: occ.start, ends_at: occ.end });
...
const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const errors = await checkConflicts({ ...visit, starts_at: newStart, ends_at: newEnd }, visitId);
