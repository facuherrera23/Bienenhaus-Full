 ...
 * Node/TypeScript version (adapted from Deno edge function).
 */
...
export interface VisitReminderRow {
    id: number;
    type: string;
    trigger_minutes_before: number | null;
    is_sent: boolean;
    visit: VisitRow | null;
}
...
export async function checkInWithQr(code: string, userId: string): Promise<CheckInResult> {
    ...
    const visit = checkin.visit as VisitRow | null;
    ...
    await supabaseAdmin.from('visits').update({ status: 'en_curso' }).eq('id', checkin.visit_id);
...
export async function processReminders(
    ...
    for (const reminder of (reminders as VisitReminderRow[] | null) || []) {
        try {
            const visit = reminder.visit as VisitRow | null;
            if (!visit || visit.deleted_at || ['cancelada', 'no_show'].includes(visit.status))
                continue;
