...
describe('Spinner', () => {
    ...
    it.each(['primary', 'white', 'inherit'] as const)('applies the %s color class', (color) => {
    ...
    it('allows overriding aria-label', () => {
        ...
        expect(screen.getByTestId('sp')).toHaveAttribute('aria-label', 'Loading content');
    });
    ...
    it('combines size, color, inline, and custom className together', () => {
        const { container } = render(<Spinner size="lg" color="white" inline className="extra" />);
        const spinner = container.firstChild as HTMLElement;
