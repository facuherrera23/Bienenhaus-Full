...
describe('RadioGroup', () => {
    ...
    it('blocks all options when the whole group is disabled', () => {
        const onChange = vi.fn();
        render(
            <RadioGroup aria-label="fruits" options={baseOptions} disabled onChange={onChange} />,
        );
        const radios = screen.getAllByRole('radio');
        for (const radio of radios) {
    ...
    it('smoke renders both sizes and both layouts without throwing', () => {
        expect(() =>
            render(<RadioGroup aria-label="sm" options={baseOptions} size="sm" />),
        ).not.toThrow();
        expect(() =>
            render(<RadioGroup aria-label="md" options={baseOptions} size="md" />),
        ).not.toThrow();
        expect(() =>
            render(<RadioGroup aria-label="stacked" options={baseOptions} layout="stacked" />),
        ).not.toThrow();
        expect(() =>
            render(<RadioGroup aria-label="inline" options={baseOptions} layout="inline" />),
        ).not.toThrow();
    });
});
