...
describe('Textarea', () => {
    ...
    it('renders a textarea with the given placeholder', () => {
        ...
        expect(screen.getByPlaceholderText('Describe la propiedad')).toBeInTheDocument();
    });
    ...
    it('applies the resize-none class when resize="none"', () => {
        const { container } = render(<Textarea placeholder="Notas" resize="none" />);
    ...
    it('applies the resize-both class when resize="both"', () => {
        const { container } = render(<Textarea placeholder="Notas" resize="both" />);
        const wrapper = container.firstChild as HTMLElement;
    ...
    it('shows the counter reflecting the controlled value length', () => {
        render(<Textarea placeholder="Notas" maxLength={10} showCounter value="hola" />);
        expect(screen.getByText('4/10')).toBeInTheDocument();
    });

    it('applies the danger counter class when the value reaches the limit', () => {
        const { container } = render(
            <Textarea placeholder="Notas" maxLength={5} showCounter value="abcde" />,
        );
    ...
    it('does not apply the danger counter class when below the limit', () => {
        const { container } = render(
            <Textarea placeholder="Notas" maxLength={10} showCounter value="abc" />,
        );
        const counter = container.querySelector('[aria-hidden="true"]');
    ...
    it('sets aria-invalid when error is true', () => {
        ...
        expect(screen.getByPlaceholderText('Notas')).toHaveAttribute('aria-invalid', 'true');
    });

    it('does not set aria-invalid when error is false', () => {
        render(<Textarea placeholder="Notas" />);
        expect(screen.getByPlaceholderText('Notas')).not.toHaveAttribute('aria-invalid');
    });
    ...
    it('updates the counter when the controlled value changes', () => {
        const { rerender } = render(
            <Textarea placeholder="Notas" maxLength={10} showCounter value="ab" />,
        );
        expect(screen.getByText('2/10')).toBeInTheDocument();
        rerender(<Textarea placeholder="Notas" maxLength={10} showCounter value="abcdef" />);
    ...
    it('uses explicit aria-label when provided', () => {
        render(<Textarea placeholder="Mensaje" aria-label="Campo de mensaje" />);
        expect(screen.getByLabelText('Campo de mensaje')).toBeInTheDocument();
    });

    it('applies size classes', () => {
        const { container, rerender } = render(<Textarea placeholder="Notas" size="sm" />);
        const wrapper = container.firstChild as HTMLElement;
    ...
    it('applies the maxLength attribute to the textarea', () => {
        ...
        expect(screen.getByPlaceholderText('Notas')).toHaveAttribute('maxlength', '50');
    });
});
