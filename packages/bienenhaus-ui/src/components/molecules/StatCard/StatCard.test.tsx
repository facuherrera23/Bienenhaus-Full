...
describe('StatCard', () => {
    ...
    it('renders an SVG polyline with the correct point count', () => {
        ...
        const { container } = render(<StatCard {...defaultProps} sparkline={data} />);
    ...
    it('does not render a sparkline with fewer than 2 points', () => {
        const { container } = render(<StatCard {...defaultProps} sparkline={[42]} />);
    ...
    it('renders an action button with the provided aria-label', () => {
        render(<StatCard {...defaultProps} action={{ label: 'Ver detalle', onClick: () => {} }} />);
    ...
    it('triggers action.onClick when the action button is clicked', () => {
        const onClick = vi.fn();
        render(<StatCard {...defaultProps} action={{ label: 'Configurar', onClick }} />);
    ...
    it('hides the trend while loading', () => {
        const { container } = render(
            <StatCard {...defaultProps} trend={{ value: '+12%', direction: 'up' }} loading />,
        );
    ...
    it('hides the sparkline while loading', () => {
        const { container } = render(<StatCard {...defaultProps} sparkline={[1, 2, 3]} loading />);
        expect(container.querySelector('polyline')).toBeNull();
    });
    ...
    it('merges a consumer className alongside module classes', () => {
        const { container } = render(<StatCard {...defaultProps} className="my-extra" />);
