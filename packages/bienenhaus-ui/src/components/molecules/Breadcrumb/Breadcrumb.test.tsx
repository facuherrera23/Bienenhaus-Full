...
describe('Breadcrumb', () => {
    ...
    it('does not collapse when items.length === maxItems', () => {
        ...
        expect(
            screen.queryByRole('button', { name: 'Mostrar anteriores' }),
    ...
    it('truncates long labels via title attribute and keeps single-line label', () => {
        const longLabel =
            'Esta es una etiqueta de breadcrumb extremadamente larga que supera los 28 caracteres';
    ...
    it('merges a consumer className onto the nav element', () => {
        const { container } = render(<Breadcrumb items={twoItems} className="my-extra" />);
