...
describe('SearchInput', () => {
    ...
    it('uses explicit aria-label when provided', () => {
        render(<SearchInput placeholder="Buscar" aria-label="Campo de búsqueda" />);
    ...
    it('respects a custom debounceMs', () => {
        const onChange = vi.fn();
        render(<SearchInput placeholder="Search" onChange={onChange} debounceMs={500} />);
        const input = screen.getByPlaceholderText('Search');
    ...
    it('fires immediately when debounceMs is 0', () => {
        const onChange = vi.fn();
        render(<SearchInput placeholder="Search" onChange={onChange} debounceMs={0} />);
        const input = screen.getByPlaceholderText('Search');
    ...
    it('shows the spinner and hides the clear button while loading', () => {
        const { container } = render(<SearchInput placeholder="Search" loading value="abc" />);
        // Clear button is hidden while loading.
        expect(screen.queryByLabelText('Limpiar búsqueda')).not.toBeInTheDocument();
        // A spinner SVG exists (the clear icon has no circle; the spinner does).
        const svgs = container.querySelectorAll('svg');
        const hasSpinner = Array.from(svgs).some((svg) => svg.querySelector('circle'));
    ...
    it('blocks typing when disabled', () => {
        const onChange = vi.fn();
        render(<SearchInput placeholder="Search" disabled onChange={onChange} />);
        const input = screen.getByPlaceholderText('Search') as HTMLInputElement;
    ...
    it('applies size classes', () => {
        const { container, rerender } = render(<SearchInput placeholder="Search" size="sm" />);
    ...
    it('cleans up the debounce timer on unmount (no stale updates)', () => {
        const onChange = vi.fn();
        const { unmount } = render(<SearchInput placeholder="Search" onChange={onChange} />);
        const input = screen.getByPlaceholderText('Search');
